...
(defn create-scene []
  (let [scene (THREE.Scene.)
        camera (THREE.PerspectiveCamera. 50 (/ 640 360) 0.1 10000)
        renderer (THREE.WebGLRenderer.)
        point-light (THREE.PointLight. 0xF8D898)
    ...
    (rf/dispatch [:initialize-db {:threejs-objects {:renderer renderer
                                                    :scene scene
                                                    :camera camera}}]))

;;   (rf/dispatch [:add-to-scene ball])
  )
